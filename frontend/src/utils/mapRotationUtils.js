...
export const calculateRouteBearing = (routeData) => {
	if (!routeData || !routeData.polyline?.geometry?.coordinates || routeData.polyline.geometry.coordinates.length < 2) {
		return 0;
	}
...
export const calculateCurrentDirection = (currentLocation, routeData) => {
	...
	for (let i = 0; i < coordinates.length; i++) {
		...
		const distance = Math.sqrt(
			Math.pow(coord[1] - currentLocation.lat, 2) + 
			Math.pow(coord[0] - currentLocation.lng, 2)
		);
