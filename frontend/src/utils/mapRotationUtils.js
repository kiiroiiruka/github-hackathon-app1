...
export const calculateCurrentDirection = (currentLocation, routeData) => {
	...
	for (let i = 0; i < coordinates.length; i++) {
		...
		const distance = Math.sqrt(
			(coord[1] - currentLocation.lat) ** 2 +
				(coord[0] - currentLocation.lng) ** 2,
		);
	...
	const nextPoint =
		coordinates[Math.min(closestIndex + 1, coordinates.length - 1)];
 ...
 * @param {Function} setMapRotation - 地図回転角度を設定する関数
 */
export const rotateMapToCurrentDirection = (
	currentLocation,
	routeData,
	setMapRotation,
) => {
