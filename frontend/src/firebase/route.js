	...
	try {
		...
		const isDuplicate = existingRoutes.some(
			(route) =>
				...
				route.destination &&
				Math.abs(
					route.departure.coordinates[0] - routeData.departure.coordinates[0],
				) < 0.000001 &&
				Math.abs(
					route.departure.coordinates[1] - routeData.departure.coordinates[1],
				) < 0.000001 &&
				Math.abs(
					route.destination.coordinates[0] -
						routeData.destination.coordinates[0],
				) < 0.000001 &&
				Math.abs(
					route.destination.coordinates[1] -
...
export const removeAllUserRoutes = async (userId) => {
	...
	try {
		...
		const deletePromises = routes.map((route) => removeUserRoute(route.id));

		await Promise.all(deletePromises);
		console.log(
			`ユーザー ${userId} の保存ルートを全削除:`,
			routes.length,
			"件",
		);
...
export const formatRouteData = (
	...
	return {
		routeName:
			routeName ||
			`${departure.name || "出発地"} → ${destination.name || "目的地"}`,
		departure: {
