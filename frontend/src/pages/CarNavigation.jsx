...
const CarNavigation = () => {
	...
	const sendLocationToFirebase = async (location) => {
		...
		try {
			...
			await update(
				ref(rtdb, `rooms/${roomId}/memberLocations/${currentUserUid}`),
				locationData,
			);
	...
	const updateRouteStatus = async (source = "timer") => {
		...
		// 高精度の最短距離で判定（20m閾値）
		const { distance } = calculateShortestDistanceToRoute(
			currentLocation,
			routeData,
		);
		...
		if (!onRoute) {
			...
			if (last) {
				const movedFromLastStart = getDistanceMeters(
					currentLocation,
					last.start,
				);
				...
				if (locations) {
					...
					Object.entries(locations).forEach(([uid, locationData]) => {
						...
						if (uid !== currentUserUid && locationData) {
							// 5分以内の位置情報のみ有効
							const isRecent =
								Date.now() - locationData.timestamp < 5 * 60 * 1000;
							if (isRecent) {
								locationMap.set(uid, {
							...
							<div className="space-y-2">
								{members
									...
									.map((member, index) => {
										...
										const isOnline =
											memberLocation &&
											Date.now() - memberLocation.timestamp < 5 * 60 * 1000;
										const isFocused =
											focusedMember && focusedMember.uid === member.uid;
										...
										return (
											...
											>
												<img
													src={
														member.photoURL &&
														!member.photoURL.includes("ui-avatars.com")
															? member.photoURL
												...
												<div className="flex-1">
													...
													{memberLocation ? (
														<div className="text-xs text-gray-500">
															{memberLocation.isUsingMockLocation
																? "テスト位置"
																: "GPS位置"}{" "}
															-
															{new Date(
															...
															<div className="text-xs text-gray-400 mt-1">
																{memberLocation.lat.toFixed(4)},{" "}
																{memberLocation.lng.toFixed(4)}
															</div>
														</div>
													) : (
							...
							{restPoint && (
								<div className="mt-3 flex items-center justify-between">
									<div className="text-xs text-gray-600">
										現在の休憩地点: {restPoint.lat.toFixed(4)},{" "}
										{restPoint.lng.toFixed(4)}
									</div>
									<button
					...
					{routeData && routeData.polyline?.geometry?.coordinates ? (
						...
						>
							...
							>
								...
								<MapClickHandler
									enabled={activeTab === "rest"}
									onClick={handleRestPointSelected}
								/>
								...
								{restPoint && restRoute && restRoute.length > 0 && (
									<Polyline
										positions={getOptimizedCoordinates(restRoute, mapZoom).map(
													...
													<img
														src={(() => {
															...
															return currentMember?.photoURL &&
																!currentMember.photoURL.includes(
																	"ui-avatars.com",
																)
																? currentMember.photoURL
								...
								{/* メンバーの位置マーカー（丸型） */}
								{Array.from(memberLocations.entries()).map(
									([uid, location]) => {
										const member = members.find((m) => m.uid === uid);
										if (!member) return null;

										const isFocused =
											focusedMember && focusedMember.uid === uid;
										...
										const memberIcon = createRoundedImageIcon(
											url,
											isFocused ? 48 : 32,
										);
