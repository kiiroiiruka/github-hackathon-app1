		...
		if (!isLoggedIn || !userUid) {
			...
			const isDuplicate = savedRoutes.some(
				(route) =>
					...
					Math.abs(route.destination.coordinates[0] - destination[0]) <
						0.000001 &&
					Math.abs(route.destination.coordinates[1] - destination[1]) <
						0.000001,
			);
									...
									<RoutingControl
										...
										onRouteInfo={(info) => {
											if (!routeCalculatedRef.current) {
												...
												console.log(
													"到着予定:",
													info.arrivalTime.toLocaleTimeString(),
												);
