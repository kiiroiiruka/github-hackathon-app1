        ...
        <Button
          ...
          onClick={async () => {
            ...
            try {
              ...
              // 詳細なルート計算を試行（オプション）
              if (selectedLocation && selectedDeparture && false) {
                // 無効化
                // ルート計算（OSRM API使用）
                try {
              ...
              console.log("🔍 保存前のroomData確認:", {
                ...
                routeDataSizeKB: routeData
                  ? Math.round((JSON.stringify(routeData).length / 1024) * 100) / 100
                  : 0,
                routeDataSizeMB: routeData
                  ? Math.round((JSON.stringify(routeData).length / (1024 * 1024)) * 100) / 100
                  : 0,
                routeDataKeys: routeData ? Object.keys(routeData) : null,
                fullRoomDataSize: JSON.stringify(roomData).length,
                fullRoomDataSizeKB:
                  Math.round((JSON.stringify(roomData).length / 1024) * 100) / 100,
                fullRoomDataSizeMB:
                  Math.round((JSON.stringify(roomData).length / (1024 * 1024)) * 100) / 100,
              });
              ...
              console.log("🔍 保存後のFirebaseデータ確認:", {
                ...
                routeDataSize: savedData?.routeData
                  ? JSON.stringify(savedData.routeData).length
                  : 0,
