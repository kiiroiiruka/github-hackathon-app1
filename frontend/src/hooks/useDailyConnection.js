...
export const useDailyConnection = (
	...
	// 参加者の状態更新をデバウンスする関数
	const debouncedParticipantUpdate = useCallback(
		(participantData, delay = 100) => {
			// 既存のタイムアウトをクリア
					...
					if (newMap.has(participantData.session_id)) {
						...
						if (!photoURL || photoURL === "" || photoURL === null) {
							...
							if (photoURL && photoURL.includes("ui-avatars.com")) {
								console.log(
									"🔄 生成されたアイコンが返されたため、既存のphotoURLを保持:",
									{
						...
						console.log(
							`🔄 デバウンス: 参加者 ${participantData.user_name} の状態を更新しました`,
							{
			...
			callObject
				.on("joined-meeting", async (event) => {
					...
					try {
								...
								if (audioElements.length === 0) {
									...
									Object.values(currentParticipants).forEach((participant) => {
										if (!participant.local && participant.audio) {
											console.log(
												`🎧 参加者 ${participant.user_name} の音声要素を作成中...`,
											);
											...
											try {
												...
												const participantTrack = remoteTracks.find(
													(track) =>
														track &&
														track.participantId === participant.session_id,
												);
												...
												if (participantTrack) {
													...
													audioElement.play().catch((error) => {
														console.warn(
															`⚠️ 参加者 ${participant.user_name} の音声再生に失敗:`,
															error,
														);
													});

													console.log(
														`✅ 参加者 ${participant.user_name} の音声要素を作成しました`,
													);
												}
											} catch (error) {
												console.error(
													`❌ 参加者 ${participant.user_name} の音声要素作成エラー:`,
													error,
												);
											}
										}
						...
						Object.entries(currentParticipants).forEach(([id, participant]) => {
							...
							let photoURL = getMemberPhotoURL(
								participant.user_name,
								participant.session_id,
							);
							...
							if (!photoURL && participant.local && currentUserUid) {
								console.log(
									"🖼️ ローカル参加者のphotoURLが見つからないため、currentUserUidで再試行:",
									{
										user_name: participant.user_name,
										session_id: participant.session_id,
						...
						setError(
							"マイクの初期化に失敗しました。ブラウザの設定を確認してください。",
						);
					}
				})
				...
				.on("participant-joined", (event) => {
					...
					if (!event.participant.local && roomId && currentUserUid) {
						...
						import("firebase/database").then(({ ref, update, get }) => {
							const memberRef = ref(
								rtdb,
								`rooms/${roomId}/members/${event.participant.session_id}`,
							);
							...
							get(memberRef)
								.then((snapshot) => {
									...
									update(memberRef, {
										...
										.then(() => {
											console.log(
												"✅ リモート参加者のFirebase membersデータを更新完了:",
												{
													photoURL: photoURL,
												},
											);
										...
										.catch((error) => {
											console.error(
												"❌ リモート参加者のFirebase membersデータ更新エラー:",
												error,
											);
										});
								})
					...
					setParticipants((prev) => {
						...
						if (photoURL) {
							setParticipants((prev) => {
								const newMap = new Map(prev);
								const existingParticipant = newMap.get(
									event.participant.session_id,
								);
							...
							setTimeout(() => {
								try {
									...
									const currentParticipant =
										currentParticipants[event.participant.session_id];
									...
									if (currentParticipant) {
										...
										if (attempts < 9) {
											...
											console.log(
												"💡 リモート参加者にマイクの許可を促してください",
											);
											console.log("📋 確認事項:");
											console.log(
												"   1. ブラウザのアドレスバー横のマイクアイコンをクリック",
											);
					...
					if (!event.participant.local && roomId && currentUserUid) {
						...
						import("firebase/database").then(({ ref, remove }) => {
							...
							remove(memberRef)
								.then(() => {
									console.log(
										"✅ リモート参加者のFirebase membersデータを削除完了",
									);

									// 参加者が離脱した後、ルーム削除チェックを実行
									setTimeout(() => {
										import("@/firebase/room").then(
											({ checkAndDeleteRoomIfEmpty }) => {
								...
								.catch((error) => {
									console.error(
										"❌ リモート参加者のFirebase membersデータ削除エラー:",
										error,
									);
								});
						});
				...
				.on("participant-updated", (event) => {
					...
					if (event.participant.local) {
						console.log("🚨 ローカル参加者の音声状態が変更されました！", {
							...
							timeSinceStart: callDuration,
							reason:
								event.participant.audio !== isMicrophoneEnabled
									? "状態不一致"
									: "状態一致",
						});
					}
					...
					if (event.participant.local) {
						...
						if (!event.participant.photoURL) {
							console.log(
								"🔄 participant-updated参加者のphotoURL再取得を試行:",
								{
									user_name: event.participant.user_name,
									session_id: event.participant.session_id,
								},
							...
							if (photoURL && !photoURL.includes("ui-avatars.com")) {
								...
								console.log(
									"🔄 participant-updated参加者のphotoURL再取得成功:",
									{
										user_name: event.participant.user_name,
										photoURL: photoURL,
									},
						...
						if (event.track && event.track.readyState === "live") {
							...
							const createAudioElement = () => {
								try {
									...
									if (
										event.track &&
										typeof event.track.getMediaStream === "function"
									) {
									...
									audioElement
										...
										.then(() => {
											console.log(
												`✅ 参加者 ${event.participant.user_name} の音声要素を作成・再生しました`,
											);
										})
										.catch((error) => {
								...
								setTimeout(
									() => {
										try {
											...
											const audioElements = document.querySelectorAll("audio");
											console.log(
												`🎵 ページ内の音声要素数: ${audioElements.length}`,
											);
											...
											if (
												callObject &&
												typeof callObject.getReceiveSettings === "function"
											) {
												callObject
													.getReceiveSettings()
										...
										if (currentParticipant.audio) {
											...
											setTimeout(() => {
												try {
													...
													if (
														event.track &&
														typeof event.track.getConstraints === "function"
													) {
														try {
													...
													if (
														event.track &&
														typeof event.track.getSettings === "function"
													) {
														try {
													...
													if (
														...
														console.log(
															"   3. ヘッドフォン/スピーカーの接続を確認",
														);
														console.log(
															"   4. 他のアプリで音声が再生されるかテスト",
														);
													}
												} catch (error) {
				...
				.on("track-stopped", (event) => {
					...
					if (event.track?.kind === "audio") {
						...
						if (!event.participant?.local) {
							console.warn(
								"💡 リモート参加者の音声が停止しました。ネットワーク接続を確認してください。",
							);
				...
				.on("error", (event) => {
					...
					if (
						...
						event.error?.includes("connection")
					) {
						...
						event.error?.includes("track") ||
						event.error?.includes("audio")
					) {
						console.error("🎵 音声トラックエラー:", event.error);
						setError(
							"音声トラックに問題があります。ページを再読み込みしてから再度試行してください。",
						);
					} else {
	...
	const requestMicrophonePermission = useCallback(async () => {
		try {
			...
			if (error.name === "NotAllowedError") {
				setError(
					"マイクのアクセスが拒否されました。ブラウザの設定でマイクを許可してください。",
				);
				...
				setError(
					"マイクが他のアプリケーションで使用中です。他のアプリケーションを閉じてから再試行してください。",
				);
			} else {
	...
	// 参加者データを配列化（デバッグログ付き）
	const participantsArray = Array.from(participants.values()).map(
		(participant) => {
			...
			if (daily && isJoined) {
				try {
					...
					Object.values(participants).forEach((participant) => {
						...
						if (!participant.local && participant.audio) {
							console.log(
								`🎧 参加者 ${participant.user_name} の音声トラックを確認中...`,
							);

							// 既存の音声要素を確認
							const existingAudio = document.querySelector(
								`audio[data-participant="${participant.session_id}"]`,
							);
							if (existingAudio) {
								console.log(
									`✅ 参加者 ${participant.user_name} の音声要素は既に存在します`,
								);
								return;
							}
							...
							if (participantTrack) {
								console.log(
									`🎵 参加者 ${participant.user_name} の音声トラックを発見:`,
									participantTrack,
								);
								...
								audioElement
									...
									.then(() => {
										console.log(
											`✅ 参加者 ${participant.user_name} の音声要素を作成・再生しました`,
										);
									})
									.catch((error) => {
										console.warn(
											`⚠️ 参加者 ${participant.user_name} の音声要素の再生に失敗:`,
											error,
										);
									});
								...
								setTimeout(() => {
									console.log(
										`🎵 参加者 ${participant.user_name} の音声要素状態:`,
										{
								...
								console.warn(
									`⚠️ 参加者 ${participant.user_name} の音声トラックが見つかりません`,
								);
							}
						}
					});
					...
					setTimeout(() => {
						...
						console.log(
							`🎵 作成された音声要素の総数: ${allAudioElements.length}`,
						);
				...
				console.warn(
					"⚠️ Daily.coインスタンスが存在しないか、ルームに参加していません",
				);
			}
		}, [daily, isJoined]),
	};
