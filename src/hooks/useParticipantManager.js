...
export const useParticipantManager = (roomId) => {
	...
	const startParticipantSession = useCallback(
		async (userId, participantInfo = {}) => {
			if (!roomId || !userId) return;

			// 既にセッションが開始されている場合はスキップ
			if (participantSessions[userId]?.isActive) {
				console.log("Participant session already active:", { roomId, userId });
				return;
			}
			...
			const sessionData = {
				...
				participantInfo: {
					name:
						participantInfo.user_name || participantInfo.name || "Anonymous",
			...
			try {
				...
				console.log("Participant session started:", {
					roomId,
					userId,
					sessionData,
				});
		...
		[roomId, participantSessions],
	);

	// 参加者のセッション終了
	const endParticipantSession = useCallback(
		async (userId, callDuration = 0) => {
			if (!roomId || !userId) return;
			...
			try {
				...
				console.log("Participant session ended:", {
					roomId,
					userId,
					callDuration,
				});
	...
	// 参加者の通話時間を更新
	const updateParticipantDuration = useCallback(
	...
	const handleParticipantUpdate = useCallback(
		async (event) => {
			switch (event.type) {
				case "participant-joined":
					// 現在のユーザーの場合はスキップ（手動で開始済み）
					if (
						event.participant.user_id === currentUserUid ||
						event.participant.session_id === currentUserUid
					) {
						console.log("Skipping participant-joined for current user");
						return;
					}
		...
		[startParticipantSession, endParticipantSession, currentUserUid],
	);

	// 現在の参加者リストを取得
	const getActiveParticipants = useCallback(() => {
		return Object.entries(participants).filter(
			([_, participant]) => participant.inCall === true,
		);
	}, [participants]);
	...
	const getCallHistory = useCallback(
		async (userId) => {
			...
			try {
				...
				return history
					? Object.entries(history).map(([timestamp, data]) => ({
